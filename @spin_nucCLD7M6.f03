...
!*     Positions of Fe(2+)/Fe(3+) at B-site are shuffled         *
!*                                                               *
!*    .............................                              *
!*               site=1   site=2                                 *
!*    .............................                              *
...
!% mpif90 -mcmodel=medium -fpic -o a.out @spin_nucCLD7M6.f03 -I/opt/fftw-3.3.10/include -L/opt/fftw-3.3.10/lib -lfftw3 &> log
!                                                                *
!% mpiexec -n 6 a.out &                                          *
!*****************************************************************
!
...
!
      if(rank.eq.0) then
        open (unit=11,file=praefixc//'.11'//suffix,form='formatted')
!
...
!       read(05,'(i1)') kstart 
!
        write(11,*) 'Give the file name by praefix and suffix'
        write(11,*) 'type praefix (a3)...'
!       read(05,'(a3)') praefix
!
        write(11,*) 'type suffix (a1)...'
!       read(05,'(a1)') suffix
!
...
!
      if(rank.eq.0) then
        open (unit=11,file=praefixc//'.11'//suffix, &              
...
!
!
        open (unit=77,file=praefixc//'.77'//suffix,form='formatted')
!
        nframe= 4
        call gopen (nframe)
        close (77)
...
!
        wtime = wtime2 -wtime1
        write(11,'(" cpu, wtime (sec)=",2f10.3)') cputime,wtime
!
        close (11)
      end if
!*
...
!--------------------------
        ...
        open (unit=11,file=praefixc//'.11'//suffix, &
                    status='unknown',position='append',form='formatted')
!
        write(11,*) 'Read: ',praefixi//'.12'//suffix
        close (11)
        end if
!
        open (unit=12,file=praefixi//'.12'//suffix, &
...
!-------------------------------------------------------
      if(rank.eq.0) then
        open (unit=21,file=praefixc//'.21'//suffix,form='formatted')
!
...
!
  ...
  910   continue
        close (21)
!
        open (unit=13,file=praefixc//'.13'//suffix,form='unformatted')
!
...
!-----------------------------------------------------------------
      subroutine init (x,y,z,ch,spx,spy,spz,sp2,spec,site, &
                       ...
                       i1x,i2x,i1y,i2y,i1z,i2z,rank,size)
!-----------------------------------------------------------------
!o 2.057265000 2.057265000 2.057265000
      use, intrinsic :: iso_c_binding
      implicit none
!
      include    'mpif.h'
...
!    dround --> = dnint (round off to a nearest integer). 
!/*---------------------------------------------------------------------
...
!
      ...
      real(C_DOUBLE) xleng0,yleng0,zleng0,t,pi,dt,tmax,cptot,  &
                     ...
                     dtwr,dtwr2
      character*4    text1*40 ! param.h -> praefix*6,suffix*1
!
      real(C_DOUBLE) rad_Fe,rad_O,elj_Fe,elj_O,rcut,rcutC
      common/atoms/  rad_Fe,rad_O,elj_Fe,elj_O,rcut,rcutC
!
...
!
      if(rank.eq.0) then
        OPEN (unit=11,file=praefixc//'.11'//suffix, &
                status='unknown',position='append',form='formatted')
!
        write(11,*) ' read: .',praefixs//'_config.START'//suffix
        write(11,*) ' write: ',praefixc//'.13'//suffix
...
! 
!     spx(j)= sp2(j)*dsin(th)*dcos(ph)
!     spy(j)= sp2(j)*dsin(th)*dsin(ph)
      spz(j)= sp2(j)*dcos(th)
!
